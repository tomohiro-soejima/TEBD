...
result: the last matrix implementation is faster by a factor of 10
when D = 1000
=#
...
function matrixForm(M,Gamma,D,D2,d)
    ...
    Gamma2 = PermutedDimsArray(reshape(Gamma,D^2,d),(2,1))
    G2 = M2*Gamma2
...
naive_for_loop(M,Gamma,D,D2,d)
A1 = @time naive_for_loop(M,Gamma,D,D2,d)
naive_for_loop_reverse_order(M,Gamma,D,D2,d)
A2 = @time naive_for_loop_reverse_order(M,Gamma,D,D2,d)
matrixForm(M,Gamma,D,D2,d)
